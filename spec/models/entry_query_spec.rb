...
describe EntryQuery do
  ...
  let!(:entry) do
    create(:entry, billed: false, project: project, date: 20.days.ago)
  end
  let!(:entry2) { create(:entry, billed: true, date: 10.days.ago) }
  let(:params) { {} }
  ...
  describe "#filter" do
    ...
    it "filters the entries on from_date" do
      expect(filter(from_date: entry.date + 1.day).count).to eq(1)
    end

    it "filters the entries on to date" do
      expect(filter(to_date: entry2.date - 1.day).count).to eq(1)
    end
    ...
    it "filters on all params" do
      expect(filter(
        {
          billed: entry.billed,
          to_date: entry2.date + 1.day,
          from_date: entry.date - 1.day,
